...
@torch.library.custom_op("fts::bincount", mutates_args={})
def compilable_bincount(x: torch.Tensor, minlength: int) -> torch.Tensor:
    return x.bincount(minlength=minlength)


@compilable_bincount.register_fake
...
def _scatter2scatter(
    ...
    grid = lambda meta: (padded_block_idxs.size(0) * triton.cdiv(meta["N"], meta["BLOCK_N"]),)  # noqa: E731
...
@torch.library.custom_op("fts::scatter2scatter", mutates_args={"out"})
def _scatter2scatter_compilable(
    X: torch.Tensor,
    W: torch.Tensor,
    sorted_expert_idxs: torch.Tensor,
    sorted_scattered_idxs: torch.Tensor,
...
def _group_bwd_W(DY: torch.Tensor, X: torch.Tensor, expert_offsets: torch.Tensor, DW: torch.Tensor, E: int) -> None:
    grid = lambda meta: (E * triton.cdiv(meta["K"], meta["BLOCK_K"]), triton.cdiv(meta["N"], meta["BLOCK_N"]))  # noqa: E731
...
# custom op is needed because of https://github.com/pytorch/pytorch/issues/136394
@torch.library.custom_op("fts::group_bwd_W", mutates_args={"DW"})
def _group_bwd_W_compilable(
...
def group_bwd_W(DY: torch.Tensor, X: torch.Tensor, expert_offsets: torch.Tensor, DW: torch.Tensor, E: int) -> None:
    if torch.compiler.is_compiling():
        _group_bwd_W_compilable(DY=DY, X=X, expert_offsets=expert_offsets, DW=DW, E=E)
    else:
        _group_bwd_W(DY=DY, X=X, expert_offsets=expert_offsets, DW=DW, E=E)
...
def _group(
    ...
    grid = lambda meta: (triton.cdiv(meta["N"], meta["BLOCK_N"]),)  # noqa: E731
...
# custom op is needed because of https://github.com/pytorch/pytorch/issues/136394
@torch.library.custom_op("fts::group", mutates_args={"out"})
def _group_compilable(
    A: torch.Tensor,
    sorted_expert_idxs: torch.Tensor,
    out: torch.Tensor,
    coeff: torch.Tensor | None = None,
...
def group(
    ...
    if torch.compiler.is_compiling():
        _group_compilable(A=A, sorted_expert_idxs=sorted_expert_idxs, out=out, coeff=coeff, fan_out=fan_out)
    else:
        _group(A=A, sorted_expert_idxs=sorted_expert_idxs, out=out, coeff=coeff, fan_out=fan_out)